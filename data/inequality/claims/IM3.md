---
id: IM3
kind: mechanism
title: Tax policy is shaped disproportionately by concentrated wealth
domain: inequality
confidence: 0.65
sources:
  - label: Saez & Zucman — The Triumph of Injustice
    url: 'https://taxjusticenow.org/'
    kind: book
    year: 2019
    authors: Saez, Zucman
    finding: Distributional national accounts showing that the effective tax rate paid by the wealthiest 400 US households fell below the rate paid by the bottom 50% in 2018, reversing a half-century pattern.
  - label: Gilens & Page — Testing Theories of American Politics
    url: 'https://www.cambridge.org/core/journals/perspectives-on-politics/article/testing-theories-of-american-politics-elites-interest-groups-and-average-citizens/62327F513959D0A304D4893B382B992B'
    kind: paper
    year: 2014
    authors: Gilens, Page
    finding: Multivariate analysis of 1,779 US policy outcomes finds elite preferences and business-group positions predict policy adoption; median-voter preferences have near-zero independent effect.
analyses:
  - A_xdom_inequality_to_authoritarianism
authoredBy:
  agent: claude-opus-4-7
  promptTitle: Seed claim author v0.1
  generatedAt: '2026-05-11T12:00:00Z'
createdAt: '2026-05-11T12:00:00Z'
---
Concentrated wealth funds the lobbying, think-tank, and campaign-finance infrastructure that shapes tax-policy design. The result is a tax system whose effective rates and structural carve-outs (capital-gains preference, step-up basis, pass-through deductions, foundation rules) favor wealth-holders disproportionately and progressively across decades — a self-reinforcing channel that distinguishes concentrated wealth from a static distributional outcome.
