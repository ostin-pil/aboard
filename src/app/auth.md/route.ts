...
export function GET() {
  ...
  const body = [
    ...
    "",
    "Some gateways decide whether a server needs authentication once, when the",
    "connection is created, and never ask again. Because reads here are public,",
    "such a client completes its handshake, records this server as needing",
    "nothing, and never runs an authorization flow. Its writes then fail with a",
    "401 it can no longer act on, and no amount of retrying helps.",
    "",
    `Point that client at ${base}/mcp?auth=required instead. The challenge`,
    "arrives at the handshake, where such a gateway expects it, so it discovers",
    "the authorization server and completes the flow before calling anything.",
    "Everything else is identical: same endpoint, same metadata document, same",
    "token audience, same scope. It changes when you are asked, not what you",
    "get. Connect directly and you want the plain URL, where reads stay free.",
    "",
