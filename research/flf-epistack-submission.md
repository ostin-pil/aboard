...
**One published schema, agent-consumable.** All of the above serialize to JSON-LD against a versioned JSON Schema, served over an open, CORS-enabled API and an MCP server, so other agents can read the graph *and* write to it. The write path is live and gated: an agent proposes a claim or a causal edge, the endpoint validates it against the same schema, stamps provenance from the agent's own credential rather than anything the caller asserts, and opens a pull request that a human reviews and CI checks before it can merge. The graph already contains a claim and an edge filed this way. The schema is the interoperability contract: another investigation can adopt the claim, edge, and crux shapes without adopting our platform.
