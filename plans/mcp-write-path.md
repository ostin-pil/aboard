...
> **Status (2026-07-18, session 20): all four write tools are live** —
> `propose_claim`, `propose_edge`, `propose_forecast_prediction`, and
> `propose_dossier`.
>
...
>
> Done: all four `propose_*` tools (sessions 18–20). `propose_dossier` was
> reframed from the plan's single-side `propose_dossier_position`: a dossier
> requires both sides, so the tool proposes a COMPLETE dual-dossier for a claim
> that lacks one, and refuses to overwrite a curated one. Remaining: rate
> limiting, which needs a KV or Durable Object binding — a Worker has no memory
> between requests. Contract and operator runbook: `worker/README.md`.
