# Code-quality audit — 2026-07-18, revised 2026-07-19 (v2)
...
## Already resolved (state on `main` today)
...
- **Split `about/page.tsx` (417) and `claims/[id]/page.tsx` (411)** by
  section, per the ~250-line page rule.
...
- **Extract `src/lib/vocab.ts` and `src/lib/tokens.ts`** as the single source
  for enums, kind→row/letter maps, and the OG palette (15 colors; drives B).
- **Add a route error boundary** (`src/app/graph/error.tsx`) — upgraded from
  nice-to-have: E2's corrupt-state crash is *persistent* without one (the
  throw precedes any `clearPersisted()`).
- **De-stale the docs (consolidated list):** CLAUDE.md:32,60,77 + README.md:74
  still describe the deleted `public/graph-engine.js` as the live renderer
  (it's `ClaimGraphRF` on @xyflow/react; `ClaimGraphCanvas` is a 50-line
  wrapper); CLAUDE.md lists `scripts/generate-prediction.ts` (retired for
  `scripts/forecasters/ensemble-predict.ts`); CLAUDE.md calls
  `cross_domain_edges.yaml` "empty in v0" (it carries CE1–CE3);
  `next.config.ts`'s SITE_URL comment contradicts `site.ts`;
  `mcp-server/src/index.ts:7-8` claims write tools are "stubbed" (all four are
  wired).
...
| # | Sev | Where | Defect |
|---|-----|-------|--------|
| E1 | HIGH | `ClaimGraphRF.tsx:102-118` + `persist.ts` | **The landing page's inline graph hydrates the `/graph` editor's localStorage sandbox.** `loadPersisted()` runs unconditionally; the self-heal check (`mode !== "fullbleed" || …`) exempts inline mode from all validation. Any visitor who ever edited on `/graph` sees their scratch state — all domains, fullbleed coordinates, deleted seeds — on the canonical marketing surface, while the header still shows counts from the real `engineData`. Fix: skip `loadPersisted()` in inline mode (or per-mode keys). |
| E2 | MED | `persist.ts:49-53` | Corrupt-but-parseable state passes the shape check (only `nodes[0]` is inspected; `edges` never) and **crashes render before any `clearPersisted()` — a persistent white-screen** on both `/graph` and (via E1) the landing page, until manual localStorage surgery. Fix: Zod-validate the payload (per the repo's own rule), try/catch → clear + rebuild; plus the D error boundary. |
| E3 | MED | `persist.ts:10` | No seed/content versioning: `aboard.graph.v3` invalidates only on a hand-bumped key, so **returning visitors never see content merged into `data/` after their first edit** — for a board whose content is the product. Fix: stamp a seed hash; on mismatch offer refresh/merge. |
| E4 | MED | `ClaimGraphRF.tsx:347-366` | Undo/redo restores group `style.width/height` without `updateNodeInternals` — reintroducing exactly the stale-measure/undraggable-pill bug the code's own comment at `:322-327` documents (and `knowledge/issues.md` records). Fix: rAF-call `updateNodeInternals` for style-changed groups after undo/redo. |
...
1. **Hotfix PR — ship first:** A1 (`metadataBase`) + A2 (three `aboard.dev`
   sites + canonical client export) + the two forbidden-string gates (source
   grep + built-output localhost check). *Accept:* `grep -rc localhost:3000
   out/ --include='*.html'` = 0; grep gates red on reintroduction.
2. **Graph-state integrity PR:** E1 (inline isolation), E2 (Zod-validated
   persistence + clear-on-corrupt), E3 (seed stamp), E4 (undo/redo
   re-measure), `graph/error.tsx` (D). One cluster, one browser-QA pass.
   *Accept:* malformed-payload unit tests green; manual QA script covering
   edit-on-/graph → landing unchanged; corrupt key self-heals.
...
6. **Refactor PRs (one each, browser-QA'd):** `ClaimGraphRF` split (now 9
   extractable units), `vocab.ts`/`tokens.ts` extraction, hover/connection
   perf (corrected targets in D), page splits, E12 exporter append-semantics,
   E16–E21 cleanups, docs de-stale (the consolidated D list).
