...
The aboard dev server must be reachable for the read tools to return data (`npm run dev` from the repo root for the default base URL). The MCP server itself starts even if aboard is down; read calls then return a clear "could not reach aboard API" error.
