...
- **Load-bearing invariant:** never `-d`/`-D` a branch absent from `git branch --merged main` taken on an up-to-date local `main`. That single fact is the only thing that makes force-delete safe.
- Preconditions are hard gates, not warnings. "Merged into main" is meaningless against a stale or dirty `main` — abort, delete nothing.
- Local-only. Never delete, prune, or push branches on `origin`. Remote pruning is deliberately out of scope; do it by hand with `git push origin --delete <branch>` if ever needed.
